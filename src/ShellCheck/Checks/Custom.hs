...
checkCompareArgsNumber params root@(T_Annotation _ _ (T_Script rootScriptID _ _)) = do
  ...
  where
    ...
    check t =
      isDirectChildOfRoot t && case t of
...
checkDevRedirect
  ...
  redir@( T_Redirecting
            ...
            _
          ) = when (isDev file) $ error file
    where
      isDev t = case t of
        T_NormalWord _ [T_Literal _ str] -> str `isPrefixOf` "/dev/sd"
        _ -> False

      error t = warn (getId t) 5109 "高危命令检测: 重定向到 /dev/sd*"
...
checkCommandResult params token =
  ...
  where
    ...
    message code id =
      when (isOnlyTestInCommand token && not isFirstCommandInFunction) $
        info id 5004 $
...
checkReadonlyAssignment params t =
  execWriter $ evalStateT (mapM analyzeAssignment assignments) Set.empty
  where
    warnFor (name, token) =
      warn (getId token) 5006 $ "赋值给只读变量" ++ name ++ "."
    ...
    isReadOnly cmd =
      maybe
        False
        ( \case
            "declare" -> any (\t -> 'r' `elem` concat (oversimplify t)) $ filter isOption $ arguments cmd
            "readonly" -> True
            _ -> False
        )
        $ getCommandName cmd

    assignments = [(name, token) | Assignment (token, _, name, _) <- variableFlow params, isVariableName name]

    isValueAssignment token = case token of
      T_Assignment {} -> True
      T_SimpleCommand _ _ cmds -> any isValueAssignment cmds
      _ -> False

    analyzeAssignment (name, token) = do
      readonly <- gets $ Set.member name
      when (readonly && isValueAssignment token) $ warnFor (name, token)
      when (isReadOnly token) $ modify $ Set.insert name

checkAssignmentLiteralOnce params t =
  execWriter . sequence $ mapMaybe warningFor assignmentTimesMap
  where
    assignments = [(name, token) | Assignment (token, _, name, _) <- variableFlow params, isVariableName name]
    assignmentMap = Map.fromList assignments
    assignmentTimesMap = Map.toList $ Map.fromListWith (+) [(name, 1) | (name, _) <- assignments]

    isDeclareCommand t@T_SimpleCommand {} = getCommandName t == Just "declare" || getCommandName t == Just "readonly"
    isDeclareCommand _ = False

    isLiteralAssignment (T_Assignment _ _ _ _ value) = isLiteral value
    isLiteralAssignment _ = False

    warningFor (name, times) = do
      guard $ times == 1
      token <- Map.lookup name assignmentMap
      guard $ (not . isDeclareCommand) token && isLiteralAssignment token
      return $ warn (getId token) 5007 "检测到只赋值一次的字面量, 建议设置为只读全局变量"
